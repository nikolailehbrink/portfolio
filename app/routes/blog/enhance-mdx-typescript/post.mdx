...
When using Vite and TypeScript, it is recommended to use the [`vite-tsconfig-paths`](https://github.com/aleclarson/vite-tsconfig-paths) plugin, which automatically resolves your `tsconfig.json` paths in `.ts` and `.tsx` files.

For example, you can use clean import paths like `@/components/Button` instead of long relative paths such as `../../components/Button`.
...
According to the [plugin's documentation](https://github.com/aleclarson/vite-tsconfig-paths?tab=readme-ov-file#%EF%B8%8F-non-typescript-modules-need-special-configuration), if you prefer not to enable `allowJs`, or it didn't help, you can pass `loose: true` to the plugin config instead:
...
{/* TODO: Add image with incorrectly passed in prop */}
...
{/* TODO: Add image of image where an error is shown, that there is an incorrect prop */}

With `checkMdx` enabled, you’ll get type errors when props don’t match expected types—just like in your `.tsx` files. This improves reliability and helps catch mistakes earlier.
