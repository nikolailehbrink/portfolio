...
<Alert type="tip">
  Keep an eye on `/context` during longer sessions. If you're past 80% and the
  responses start feeling less focused, it's time to `/compact` or `/clear`.
</Alert>
...
The [Model Context Protocol](https://modelcontextprotocol.io/) lets you connect Claude to external services — GitHub, Sentry, Slack, your database, and more.
...
<Alert type="warning">
  Each MCP server's tool definitions consume context tokens just by being
  registered. If you notice your context filling up fast, run `/context` and
  audit what's loaded. Some servers consume 8-30% of your available budget.
</Alert>
...
The `/batch` command uses this internally. When you run something like `/batch migrate src/ from CommonJS to ES modules`, it splits the work across 5-30 parallel worktrees automatically.
